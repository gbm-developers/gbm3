...
context("Check gbm2 and gbm produce same results - No CV")
test_that("Gaussian distribution is same for old gbm and new", {
  ## Based on example in R package
  ...
  # predict on the new data using "best" number of trees
  f.predict <- predict(gbm_fit_2, data2, 100) # f.predict will be on the canonical scale (logit,log,etc.)
