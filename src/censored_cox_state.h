...
public:
	...
	)
	{
		...
		LogLikelihood(kData.get_trainsize(), kData, kFuncEstimate, &martingale_resid[0], false);
	...
	)
	{
	    ...
	    loglik = LogLikelihood(kNumRowsInSet, kData, kFuncEstimate, &martingale_resid[0]);
	...
	)
	{
	    ...
	    loglike_no_adj = LogLikelihood(kData.get_trainsize(), kData, kFuncEstimate, &martingale_resid_no_adj[0], false, false);
	    loglike_with_adj = LogLikelihood(kData.get_trainsize(), kData, &eta_adj[0], &martingale_resid_with_adj[0], false, false);
...
private:
								...
								double* resid, bool skipbag=true, bool checkinbag=true)
	{
	    ...
	    for(person = 0; person < n; person++)
	    {
	    	...
	    	{
	    		new_center = eta[coxph_->EndTimeIndices()[p2]] + kData.offset_ptr()[coxph_->EndTimeIndices()[p2]];
	    	...
	    	if(skipbag || (kData.get_bag_element(p2)==checkinbag))
	    	{
				...
				if (person == coxph_->StrataVec()[istrat])
				{
					for (; indx1< coxph_->StrataVec()[istrat]; indx1++)
					{
						...
						if(skipbag || (kData.get_bag_element(p2)==checkinbag))
						{
							resid[p2] -= cumhaz * exp(eta[p2] + kData.offset_ptr()[p2] - center);
						}
					}
					cumhaz =0;
					denom = 0;
