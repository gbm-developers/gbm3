                     ...
                     par.details = getOption("gbm.parallel"))
{
  ...
  if(is.null(object$Terms) && is.null(object$data)) {
    ...
    if(object$distribution$name == "coxph") 
    {
      ...
      {
        message("strata not provided - assuming train/test sets are comprised of a single stratum each")
    ...
    y.integer <- object$data$y.integer
    w       <- object$data$w
    nTrainRows  <- object$nTrain
    nTrainPats <- object$nTrainPats
    cRows   <- length(y)
