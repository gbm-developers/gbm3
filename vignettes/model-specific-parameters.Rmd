...
library("gbm3")
```
Several distributions provided by `gbm3` have model-specific parameters. All distributions have parameters such as means or variances, but some require additional information to be fully defined. This additional information is referred to as "model-specific parameters". This document describes how to specify these parameters when constructing the associated `GBMDist` object, as well as their default values.

## Distributions with model specific parameters
Five distributions in `gbm3` have additional parameters: `CoxPH`, `Pairwise`, `Quantile`, `TDist` and `Tweedie`.

## Cox proportional hazards model
The Cox proportional hazards model accepts right-censored `Surv(time, status)` responses and counting-process `Surv(start, stop, status)` responses. It has several model-specific parameters associated with it. All of them are optional but play important roles in the boosting process.

* `strata`: a vector of positive integers indicating which strata each row of data belongs to.  If there are multiple rows per observation, this should be reflected in the `strata` vector.  If not specified it is assumed all training data are in the same stratum and all test data are in another stratum.
* `sorted`: a vector specifying how the rows of data are ordered within their `strata`; within each stratum, the order is the reverse order of the event times for right-censored data or the start times for counting-process data.  This vector is completely optional and will be calculated by `gbmt`.
...
* `prior_node_coeff_var`: a double used to regularize the model predictions in `gbm3`. It represents the prior on the number of events in the model.  The predictions of the `GBMFit` are given by $\log(\text{number of events}/\text{expected number of events})$. Both the number of events in a dataset and the model's expected number of events could be $0$ leading to non-finite behaviour.  The inverse of this parameter is added to both the numerator and denominator appearing in the log ratio so as to ensure the predictions are finite.  The default value is $1000$, representing a base event number of $1/1000$ events regardless of the value of the measured or expected number of events.
...
The default for `group` is `"query"` while `metric` defaults to `"ndcg"`. If `map` or `mrr` are selected the response must be in ${0, 1}$.  A cut-off in the ranking of items in a group can be specified via `max_rank`, the default for this is 0 (all ranks taken into account) and is only applicable for "ndcg" and "mrr".  Finally, the `group_index` or label can be specified directly - note this is optional and will be calculated by `gbmt`.
...
## TDist
The t-distribution requires its degrees of freedom (`df`) to be set. The default value for this is four but it can be specified on construction of the associated `GBMDist` object.
