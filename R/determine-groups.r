...
#' 
#' @param col_names vector of variable names used in fitting - required for Pairwise
...
determine_groups.default <- function(col_names, gbm_data, distribution_obj) {
...
determine_groups.PairwiseGBMDist <- function(col_names, gbm_data, distribution_obj) {
  ...
  distribution_obj$group_name <- factor(do.call(paste, c(gbm_data$x[,distribution_obj$group, drop=FALSE], sep=":")))
