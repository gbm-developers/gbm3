...
Figure 2 compares the three methods for estimating the optimal number of iterations across 13 datasets. The boxplots show the methods' performance relative to the best method on that dataset. For most datasets the methods perform similarly; however, 5-fold cross validation is consistently the best of them. OOB, using a 33% test set, and using a 20% test set all have datasets for which they perform considerably worse than the best method. My recommendation is to use 5- or 10-fold cross validation if you can afford the computing time. Otherwise you may choose among the other options, knowing that OOB is conservative.
...
Let $\eta_i=o_i+f(\mathbf{x}_i)$, $q_i=2y_i-1$, and $u_i=q_i\eta_i$.
...
Notes: This distribution requires $y_i\in\{0,1\}$.
...
Terminal node estimates   $\mathrm{median}_w(y-f(\mathbf{x}))$
------------------------  ---------------------------------------------------------------

Notes: $\mathrm{median}_w(y)$ denotes the weighted median, defined as the solution to the equation $\sum w_iI(y_i\leq m)=\frac{1}{2}\sum w_i$. If the cumulative weight equals $\frac{1}{2}\sum w_i$ exactly at an observed value, the two adjacent order statistics are averaged (with unit weights this matches `quantile(y, 0.5, type = 6)`, which coincides with the familiar even-$n$ median convention).
...
Notes: $\mathrm{quantile}^{(\alpha)}_w(y)$ denotes the weighted quantile, defined as the solution, *q*, to the equation $\sum w_iI(y_i\leq q)=\alpha\sum w_i$. When the cumulative weight equals $\alpha\sum w_i$ exactly at an observed value, any value between the two adjacent order statistics solves the equation; the implementation interpolates with weight $(1-\alpha)$ on the lower value and $\alpha$ on the upper value. With unit weights this matches `quantile(y, alpha, type = 6)` at these boundary points (see `?quantile` for the definitions of the nine types), and at $\alpha=1/2$ it reduces to the usual averaging convention for the median.

## t-distribution

The `TDist` distribution has a degrees-of-freedom parameter $\nu$, which defaults to 4.

------------------------  ---------------------------------------------------------------
Deviance                  $\displaystyle \frac{1}{\sum w_i}\sum_i w_i \log(\nu + (y_i-f(\mathbf{x}_i))^2)$
Initial value             location M-estimate of $y-o$
Gradient                  $\displaystyle z_i=\frac{2(y_i-f(\mathbf{x}_i))}{\nu + (y_i-f(\mathbf{x}_i))^2}$
Terminal node estimates   location M-estimate of $y-f(\mathbf{x})$
------------------------  ---------------------------------------------------------------

Notes: The location M-estimate starts from the weighted median and iterates using weights determined by the t-distribution score function.

## Cox Proportional Hazard

------------------------  ---------------------------------------------------------------
Deviance                  negative partial log likelihood
Initial value             0
Gradient                  martingale residual, multiplied by the observation weight in tree fitting
Terminal node estimates   $\displaystyle \log\left(\frac{\sum_{\mathbf{x}_i\in S_k}\delta_i + 1/c}{\sum_{\mathbf{x}_i\in S_k}\max(0,\delta_i-m_i) + 1/c}\right)$
                          where $m_i$ is the martingale residual and $c$ is `prior_node_coeff_var`
------------------------  ---------------------------------------------------------------

Notes:

- $t_i$ is the survival time for right-censored data; counting-process data use start and stop times.

- $\delta_i$ is the event indicator.

- The implementation supports Breslow and Efron tie handling, with Efron as the default.

- The implementation supports strata and both right-censored and counting-process `Surv` responses.

- See the "Guide to the Cox Proportional Hazards model" vignette for implementation details.
...
The `Gamma` distribution uses a log link. Let $\eta_i=o_i+f(\mathbf{x}_i)$.

------------------------  ---------------------------------------------------------------
Deviance                  $\displaystyle 2\frac{1}{\sum w_i}\sum_i w_i\left(y_i e^{-\eta_i}+\eta_i\right)$
Initial value             $\displaystyle \log\left(\frac{\sum w_i y_i e^{-o_i}}{\sum w_i}\right)$
Gradient                  $\displaystyle z_i=y_i e^{-\eta_i}-1$
Terminal node estimates   $\displaystyle \log\left(\frac{\sum w_i y_i e^{-\eta_i}}{\sum w_i}\right)$
------------------------  ---------------------------------------------------------------

The Gamma class bounds the most extreme values of $\eta_i$ to the interval $[-19,19]$.

## Tweedie

The `Tweedie` distribution uses a log link and has a power parameter $p$. Let $\eta_i=o_i+f(\mathbf{x}_i)$.

------------------------  ---------------------------------------------------------------
Deviance                  $\displaystyle 2\frac{1}{\sum w_i}\sum_i w_i\left(\frac{y_i^{2-p}}{(1-p)(2-p)}-\frac{y_i e^{(1-p)\eta_i}}{1-p}+\frac{e^{(2-p)\eta_i}}{2-p}\right)$
Initial value             $\displaystyle \log\left(\frac{\sum w_i y_i e^{(1-p)o_i}}{\sum w_i e^{(2-p)o_i}}\right)$
Gradient                  $\displaystyle z_i=y_i e^{(1-p)\eta_i}-e^{(2-p)\eta_i}$
Terminal node estimates   $\displaystyle \log\left(\frac{\sum w_i y_i e^{(1-p)\eta_i}}{\sum w_i e^{(2-p)\eta_i}}\right)$
------------------------  ---------------------------------------------------------------

The Tweedie class bounds the most extreme values of $\eta_i$ to the interval $[-19,19]$. In this implementation, `gbm_dist("Tweedie", power=p)` does not accept `power=0`, `power=1`, or `power=2`; these special cases raise an error directing the user to construct a `Gaussian`, `Poisson`, or `Gamma` distribution, respectively, instead.
